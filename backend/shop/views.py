from shop import app, db, api
from shop .models import Item, User, ItemResource
from shop .func import allowed_file, isForm, randomId, isFormEdit
from shop .config import UPLOAD_FOLDER, URL_BACKAND, URL_IMAGES
...
@cross_origin(origin = URL_BACKAND, headers=['Content-Type', 'Authorization'])
def index():
    ...
    # return jsonify(data)
    # return api.add_resource(ItemResource, '/api')
    return data
...
@cross_origin(origin = URL_BACKAND, headers=['Content-Type', 'Authorization'])
def get_image(id):
    image_path = os.path.join(UPLOAD_FOLDER, Item.query.get(id).image)  # Замените на фактический путь к изображению
    print(image_path)

    # image_path = url_for('display_image', filename = Item.query.get(id).image)
    # print(image_path)

    # image_path = url_for('static', filename = Item.query.get(id).image)
    # print(image_path)

    image_path = f"{URL_IMAGES}{Item.query.get(id).image}"
...
def create():
    ...
    if request.method == "POST":
        ...
        if isForm(request) and title and price:
            filename = secure_filename(file.filename)
            filename = randomId(filename)
